...
| 지표 | 수치 |
|------|------|
| 포털 정보 탐색 시간 | **~95% 단축** (5~10분 → 20초 이내) |
| 인터페이스 신청서 작성 시간 | **~70% 단축** (10분+ → 3분 이내 대화형 자동완성) |
...
- `.env` 파일 설정: 프로젝트 루트 경로에 `.env` 파일을 생성하고 다음 값을 채워야 합니다.
  ...
  ```
  > `CHAINLIT_AUTH_SECRET`은 Chainlit `password_auth_callback` 사용 시 필수입니다.  
  > 터미널에서 `chainlit create-secret` 명령으로 안전한 값을 생성할 수 있습니다.
...
├── chainlit.md                # Chainlit 채팅 화면 초기 안내 메시지
│
├── .chainlit/                 # Chainlit 프레임워크 설정 폴더
│   └── config.toml            # 테마·기능·인증 방식 등 Chainlit 앱 설정
│
...
│       ├── auth.py            # 사내 포털 로그인 / JSESSIONID 세션 관리·자동 재로그인
...
│       └── application.py     # EAI·EIGW 인터페이스 신청서 작성
│                              #   Step1(기초정보) → regTemp(상세정보) → Step3(최종 승인자)
│
└── test/                      # 단위 테스트 모음
    ...
    ├── test_login.py                    # NARU 포털 로그인 / 세션 획득 통합 테스트
...
NARU-Agent가 제공하는 4가지 핵심 기능은 다음과 같습니다.

| # | 기능 | 설명 |
|---|------|------|
| 1 | **모니터링 자동 조회** | EAI MQ 큐 적체·DB 잔여 / EIGW 오류현황·응답속도 / MCG TPS·채널 상태 실시간 조회 |
| 2 | **통계 자동 분석** | 시간별·일별·월별 통계 조회 및 증감 추이(Anomaly) 자연어 요약 |
| 3 | **신청서 자동 작성** | EAI·EIGW 인터페이스 신청 대화형 Slot Filling → Step1(기초정보)·regTemp(상세정보)·Step3(최종 승인자) 임시저장까지 자동 완성 |
| 4 | **승인 게이트** | 민감 작업(신청·저장) 실행 전 사용자 확인(Human-in-the-loop) |
...
현재 등록된 `@mcp.tool()` 수: **38개**

| 파일 | 도구 수 | 주요 도구 |
|------|--------|---------|
| `institution.py` | 1 | `search_institution_code` |
| `datetime_utils.py` | 1 | `get_date_range` |
| `monitoring_eai.py` | 2 | `get_queue_depth_monitoring`, `get_eai_if_db_remain_cnt` |
| `monitoring_eigw.py` | 5 | `get_eigw_online_error_list` 외 4개 |
| `monitoring_mcg.py` | 4 | `get_mcg_chnl_status_all` 외 3개 |
| `statistic_eai.py` | 3 | `get_statistic_hourly/daily/monthly_eai` |
| `statistic_eigw.py` | 3 | `get_statistic_hourly/daily/monthly_eigw` |
| `statistic_mcg.py` | 5 | `get_statistic_hourly/daily/monthly_mcg` 외 2개 |
| `application.py` | 14 | `create_interface_request_step1`, `save_eai_interface_request_regtemp` 외 12개 |
...
Chainlit 로그인 페이지 (ID/PW 입력)
  ...
  → auth.py os.environ.get()으로 읽어 로그인 → JSESSIONID 쿠키 획득
```
...
- NARU 포털 API는 `JSESSIONID` 쿠키 기반 세션을 사용합니다. `get_session()`은 probe 요청 없이 기존 세션을 즉시 반환하며, API 응답 **401 수신 시에만** `refresh_session()`을 호출하여 자동 재로그인합니다.
...
| `test_login.py` | — | NARU 포털 로그인 API 호출 및 JSESSIONID 세션 획득 통합 검증 |
...
## 8. 핵심 기술 선택 이유 (Technology Rationale)

각 핵심 기술을 선택한 배경과 실제 활용 방식을 정리합니다.

### ① LangGraph StateGraph — Agentic Workflow 설계

**선택 이유:**  
단순 ReAct Agent는 "하나카드 EAI 오류 → 평소 대비 비교 → 통계 요약"처럼 4단계 이상의 Long-horizon task에서 이전 도구 결과를 잊고 동일 Tool을 반복 호출하거나 종료 시점을 놓치는 문제가 있었습니다.

**활용 방식:**  
`StateGraph`로 **Router → Planner → Executor** 3-Node 구조를 설계하여 `AgentState`(TypedDict)로 대화 히스토리·계획·오류·루프 상태를 명시적으로 관리합니다. `interrupt_before=["executor"]` 설정으로 Human-in-the-loop를 LangGraph 네이티브로 구현하여 별도 제어 코드 없이 안전한 승인 게이트를 달성했습니다.

### ② FastMCP — 도구 플러그인 아키텍처

**선택 이유:**  
사내 EAI·EIGW·MCG 시스템별로 모니터링/통계/신청 API가 38개 이상이며, 향후 도구 추가 시 코어 에이전트 코드를 수정하지 않고 확장할 수 있는 구조가 필요했습니다.

**활용 방식:**  
`@mcp.tool()` 데코레이터만 달면 자동 등록되는 플러그인 구조를 `mcp_server/tools/` 하위에 구현했습니다. stdio transport 서브프로세스로 계정 정보(`NARU_USER_ID`, `NARU_USER_PW`)를 환경변수 주입 방식으로 격리하여 보안성을 확보했습니다. `MultiServerMCPClient`를 `cl.user_session`에 바인딩하여 GC 방지 및 세션 유지를 달성했습니다.

### ③ Semantic Router — LLM 토큰 비용 최적화

**선택 이유:**  
전체 38개 도구를 매 요청마다 LLM에 바인딩하면 context window에 73,700 input tokens가 소비되어 응답 비용과 속도 모두 비효율적입니다.

**활용 방식:**  
Router Node에서 사용자 질문을 `eai / eigw / mcg / apply` 도메인으로 분류하고, `CATEGORY_PREFIXES` 맵 기반으로 도메인 관련 도구 8~15개만 동적 선택합니다. 그 결과 **Input 토큰 62.2% 절감** (73,700 → 27,888)을 실측 달성했습니다.

---

## 9. 핵심 문제 해결 사례 (Engineering Decisions)
